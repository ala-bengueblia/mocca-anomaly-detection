...
from trainers.train_casia1 import pretrain, train, test
from utils import set_seeds, get_out_dir, purge_ae_params
from models.casia1_model import CASIA1_Autoencoder, CASIA1_Encoder
...
def main(args):
    ...
    data_manager = CASIADataManager(
        ...
        image_size=(256, 256),  # Adapté pour CASIA1
        test_only=args.test
    )
    ...
    ae_checkpoint = None
    if args.pretrain:
        out_dir = os.path.join(args.output_path, f'casia1_class_{args.normal_class}')
        tb_writer = SummaryWriter(log_dir=os.path.join(out_dir, 'pretrain_logs'))
        
        ae_net = CASIA1_Autoencoder(args.code_length).to(device)
    ...
    if args.train:
        ...
        encoder = CASIA1_Encoder(args.code_length).to(device)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CASIA1 Anomaly Detection')
    
    # Dataset
    parser.add_argument('--data-path', default='./casia1', help='Path to CASIA1 dataset')
